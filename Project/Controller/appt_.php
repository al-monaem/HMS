<?php
  ...
  $output = '';  
    $query = "SELECT apid,symp,uname,date,remarks,contact FROM Patients,appointmentdetails where patients.id=appointmentdetails.pid and visitstatus='no' order by date desc";
    ...
    else
    {
      while ($row = $data->fetch_assoc()) {
        $output .= '<tr>
          ...
          <td>'.$row["date"].'</td>
          <td>'.$row["contact"].'</td>
          <td><a href="apttdelete.php?id='.$row['apid'].'">Delete</a></td>
        </tr>';
      }
    }
 $output .= '</table>';
